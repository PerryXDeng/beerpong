...
where $y_{ij}$ and $\epsilon_{ij}$ are the accuracy and random error for the $i$th treatment and $j$th subject respectively, $\mu$ is the mean accuracy, $\tau_{i}$ is the $i$th treatment effect, $\beta_{j}$ is the $j$th block effect, $1\leq i\leq3$, $1\leq j\leq4$. We can set up null hypothesis $$H_0: \tau_1 = \tau_2 = \tau_3$$
