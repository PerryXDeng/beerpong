---
title: "Effects of BAC Levels on Beerpong Performance"
author: "Perry Deng"
date: "11/12/2020"
output: 
  html_document:
...
There is a legend known as Ballmer Peak, a reference to Steve Ballmer of Microsoft, that consuming a moderate amount of alcohol can improve cognitive ability (see illustration from xkcd in above figure). [Jarosz et al.](https://www.sciencedirect.com/science/article/pii/S1053810012000037) shows that intoxicated men with BAC of approximate 0.075 outperform somber subjects on creative problem solving. However it does not show whether there exists an optimal intoxication level. In other words, is there an optimal intoxication level between "buzzed", "tipsy", and "sh\*\*-faced"? In this study we attempt to answer through this question by collecting experimental data on beer pong. Beer pong is popular drinking game among young adults in which ping pong balls are thrown like basketballs into a setup of plastic red SOLO cups some distance away (for more details, consult [wikihow](https://www.wikihow.com/Play-Beer-Pong)). We obtain 12 measurements from 4 subjects under 3 treatments with differing intoxication levels (0.03, 0.06, and 0.10 intended BAC), and detect statistically significant differences between treatments. Subjects appear to perform best when they are tipsy around 0.06 BAC. This suggests that there is an optimal strategy for playing beer pong by consuming "just the right amount" of alcohol. In addition, we also find evidence that beer pong may bore its participants if played daily.
...
We then check that residuals have about the same variance with regard to each treatment level and subject. We see that there is no large deviations in between residual variances of different dosages. While it looks like high dosage produce slightly smaller residuals, as do subject B, the deviations are not large enough in magnitude to be considered statistically signifcant by Levene's Test (p=0.69, p=0.52).
```{r, message=FALSE, fig.align='center', results='hide'}
...
## Summary
To summarize, we find strong evidence that there is an optimal strategy for consuming alcohol to achieve the best performance on beer pong games, and it may not be consuming as much as possible or as little as possible, but somewhere in between. We also find evidence that beer pong is a boring game that should not be played too frequently.